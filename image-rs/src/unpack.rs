...
pub async fn unpack<R: AsyncRead + Unpin>(input: R, destination: &Path) -> Result<()> {
    ...
    while let Some(file) = entries.next().await {
        ...
        if !convert_whiteout(&file.path()?, uid, gid, destination)? {
            continue;
        }
        ...
        println!("mtime = {mtime}, path = {file_path}");
...
fn convert_whiteout(path: &Path, uid: u32, gid: u32, destination: &Path) -> Result<bool> {
...
#[cfg(test)]
mod tests {
    use std::os::unix::fs::{chown, lchown, FileTypeExt, MetadataExt};
