...
pub type Result<T> = std::result::Result<T, EncMeshError>;

#[derive(Error, Debug)]
pub enum EncMeshError {

    #[error("Encrypted mesh setup failed: {0}")]
    Setup(String),

    #[error("Mesh netmask (user-configured) and worker netmask (assigned by \
