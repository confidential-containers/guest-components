...
use crate::{
    ...
    protos::{
        api::{
            ...
            InitOverlayNetworkRequest, InitOverlayNetworkResponse,
            SecureMountRequest, SecureMountResponse, UnsealSecretInput, UnsealSecretOutput,
            
        },
        api_ttrpc::{
            OverlayNetworkService, GetResourceService, ImagePullService,
            SealedSecretService, SecureMountService,
        },
...
impl OverlayNetworkService for Server {
    async fn init_overlay_network(
        ...
        let _resource = self.hub
            .init_overlay_network(req.pod_name, req.lighthouse_pub_ip)
            .await
            .map_err(|e| {
                let detailed_error = format_error!(e);
