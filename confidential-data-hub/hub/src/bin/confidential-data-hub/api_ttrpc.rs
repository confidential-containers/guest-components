...
#[derive(Clone)]
pub struct GetPublicKeyServiceClient {
    client: ::ttrpc::r#async::Client,
}
...
impl GetPublicKeyServiceClient {
    ...
    pub async fn get_public_key(&self, ctx: ttrpc::context::Context, req: &super::api::GetPublicKeyRequest) -> ::ttrpc::Result<super::api::GetPublicKeyResponse> {
        let mut cres = super::api::GetPublicKeyResponse::new();
        ::ttrpc::async_client_request!(self, ctx, req, "api.GetPublicKeyService", "GetPublicKey", cres);
    }
}

struct GetPublicKeyMethod {
    service: Arc<Box<dyn GetPublicKeyService + Send + Sync>>,
}

#[async_trait]
impl ::ttrpc::r#async::MethodHandler for GetPublicKeyMethod {
    async fn handler(&self, ctx: ::ttrpc::r#async::TtrpcContext, req: ::ttrpc::Request) -> ::ttrpc::Result<::ttrpc::Response> {
        ::ttrpc::async_request_handler!(self, ctx, req, api, GetPublicKeyRequest, get_public_key);
    }
}

#[async_trait]
pub trait GetPublicKeyService: Sync {
    async fn get_public_key(&self, _ctx: &::ttrpc::r#async::TtrpcContext, _: super::api::GetPublicKeyRequest) -> ::ttrpc::Result<super::api::GetPublicKeyResponse> {
        Err(::ttrpc::Error::RpcStatus(::ttrpc::get_status(::ttrpc::Code::NOT_FOUND, "/api.GetPublicKeyService/GetPublicKey is not supported".to_string())))
    }
}

pub fn create_get_public_key_service(service: Arc<Box<dyn GetPublicKeyService + Send + Sync>>) -> HashMap<String, ::ttrpc::r#async::Service> {
    let mut ret = HashMap::new();
    let mut methods = HashMap::new();
    let streams = HashMap::new();

    methods.insert("GetPublicKey".to_string(),
                    Box::new(GetPublicKeyMethod{service: service.clone()}) as Box<dyn ::ttrpc::r#async::MethodHandler + Send + Sync>);

    ret.insert("api.GetPublicKeyService".to_string(), ::ttrpc::r#async::Service{ methods, streams });
    ret
}
