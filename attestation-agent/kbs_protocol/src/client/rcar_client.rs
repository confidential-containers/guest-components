...
impl KbsClient<Box<dyn EvidenceProvider>> {
    async fn build_request(&self, attestation_policy_selector: Option<&str>) -> Result<Request> {
        ...
        let tee_topology = self
            .provider
            .get_tee_topology()
            .await
            .map_err(|e| Error::RcarHandshake(format!("failed to get request tees: {e}")))?;
        Ok(Request {
...
mod test {
    ...
    use crate::{
        ...
        evidence_provider::{MockedEvidenceProvider, NativeEvidenceProvider},
    };

    use crate::client::rcar_client::{
        ATTESTATION_POLICY_SELECTOR_JSON_KEY, DEFAULT_HASH_ALGORITHM, KBS_PROTOCOL_VERSION, Result,
        SELECTED_HASH_ALGORITHM_JSON_KEY, SUPPORTED_HASH_ALGORITHMS_JSON_KEY, get_hash_algorithm,
    ...
    async fn test_build_request(#[case] attestation_policy_selector: Option<&str>) {
        ...
        assert_eq!(request.tees.primary.name, Tee::Sample);
        assert_eq!(request.tees.additional.len(), 0);
