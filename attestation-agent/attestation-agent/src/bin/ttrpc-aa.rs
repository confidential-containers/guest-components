...
",
    ...
    clean_previous_sock_file(&sock_path).context("clean previous attestation socket file")?;
...
fn clean_previous_sock_file(path: &str) -> Result<()> {
    let p = Path::new(path);
    if !p.exists() {
        return Ok(());
    }
    ...
    if std::os::unix::fs::FileTypeExt::is_socket(&file_type) {
        std::fs::remove_file(p).context("remove previous socket file")?;
    } else {
        bail!(
