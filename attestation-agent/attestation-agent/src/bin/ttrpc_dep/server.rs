...
use protos::{
    ...
    ttrpc::aa::{
        attestation_agent::{
            ...
            GetEvidenceRequest, GetEvidenceResponse, GetTeeTopologyRequest, GetTeeTopologyResponse,
...
impl AttestationAgentService for AA {
    ...
    async fn get_tee_topology(
        &self,
        _ctx: &::ttrpc::r#async::TtrpcContext,
        _req: GetTeeTopologyRequest,
    ) -> ::ttrpc::Result<GetTeeTopologyResponse> {
        debug!("AA (ttrpc): get tee topology ...");

        let tee_topology = self.inner.get_tee_topology().map_err(|e| {
            error!("AA (ttrpc): get tee topology failed:\n {e:?}");
            let mut error_status = ::ttrpc::proto::Status::new();
            error_status.set_code(Code::INTERNAL);
            error_status.set_message(format!(
                "[ERROR:{AGENT_NAME}] AA-KBC get tee topology failed"
            ));
            ::ttrpc::Error::RpcStatus(error_status)
        })?;
        let mut reply = GetTeeTopologyResponse::new();
