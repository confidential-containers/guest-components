    ...
    pub fn unwrap_key(
        ...
        match wrapping_algorithm {
            KeyWrapAlgorithm::EcdhEsA256Kw => {
                ...
                let public_key = P256PublicKey::from_encoded_point(&epk_point)
                    .into_option()
...
impl P256EcKeyPair {
    ...
    pub fn x(&self) -> Result<Vec<u8>> {
        let x = EncodedPoint::from(self.public_key)
            .x()
            .ok_or(anyhow!("invalid public key: without coordicate X"))?
            .to_vec();
        Ok(x)
    }
    ...
    pub fn y(&self) -> Result<Vec<u8>> {
        let y = EncodedPoint::from(self.public_key)
            .y()
            .ok_or(anyhow!("invalid public key: without coordicate Y"))?
            .to_vec();
        Ok(y)
    }
}
