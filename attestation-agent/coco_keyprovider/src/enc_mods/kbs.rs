...
pub(crate) async fn register_kek(
    ...
    // The status MUST be checked. `?` above only covers transport errors, so without this a
    // KBS that answered 401 (an expired or wrong signing key), 403 or 5xx was reported to the
    // caller as a successful registration -- and the caller then logs "register KEK
    // succeeded." The image is still encrypted and pushed, so the publish completes with NO
    // KEY IN THE KBS, and the failure only surfaces much later inside the guest, after a
    // successful attestation, when the key release finds nothing to release.
...
mod tests {
    ...
    /// The regression this module exists for: a rejected registration must be an error.
    /// Before the status check it returned `Ok(())`, so a publish completed with no key in
    /// the KBS and nothing anywhere reported a problem.
    #[tokio::test]
