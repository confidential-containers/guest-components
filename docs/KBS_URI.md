...
To uniquely identify every resource/key in the CoCo Key Broker System, a __KBS URI__ is defined.

## Specification

The format of a KBS URI is
...
Here,
- `kbs://`: a fixed scheme, indicating that this url is a KBS URI and used in CoCo Key Broker System.
...
## How Different KBC/KBS uses a KBS URI

### CC-KBC

CC-KBC will convert a KBS URI into a web link to request a [coco-KBS](https://github.com/confidential-containers/kbs). The web link follows the [OpenAPI of KBS](https://github.com/confidential-containers/kbs/blob/main/docs/kbs.yaml#L74).
For example, a KBS URI `kbs://example.cckbs.org/alice/decryption-key/1` will be converted to `http://example.cckbs.org/kbs/v0/resource/alice/decryption-key/1`.
